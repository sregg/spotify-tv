...
public class NewSpotifyCardView extends BaseCardView {
    ...
    public NewSpotifyCardView(Context context) {
        this(context, null);
    }

    public NewSpotifyCardView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.imageCardViewStyle);
    }

    public NewSpotifyCardView(Context context, int styleResId) {
        super(new ContextThemeWrapper(context, styleResId), null, 0);
        buildImageCardView(styleResId);
    }

    public NewSpotifyCardView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void buildImageCardView(int styleResId) {
        ...
        TypedArray cardAttrs =
                getContext().obtainStyledAttributes(styleResId, R.styleable.NewSpotifyCardView);

        int cardBackgroundColor =
                cardAttrs.getInt(R.styleable.NewSpotifyCardView_spotify_card_background_color,
                        ContextCompat.getColor(context, R.color.lb_details_description_color));
        int textColor =
                cardAttrs.getInt(R.styleable.NewSpotifyCardView_spotify_card_text_color,
...
//            mBadgeView.setVisibility(GONE);
    ...
    private static int getImageCardViewStyle(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (0 == style) {
            TypedArray styledAttrs = context.obtainStyledAttributes(R.styleable.NewSpotifyCardView);
            style = styledAttrs.getResourceId(R.styleable.NewSpotifyCardView_spotify_card_theme, 0);
            styledAttrs.recycle();
        }
        return style;
    }
